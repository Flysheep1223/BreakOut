...
- **设计细节**:
  ...
  - **UI 渲染**: 在控制台绘制地图，顶部动态显示当前回合数和敌人强化等级，**如果玩家与尖刺重合，将显示'X'并触发受伤提示**。
  - **逻辑调度**: 在玩家每次行动后，依次触发物品检测、战斗判定、**尖刺伤害判定**和动态难度检查。
  - **模块化调用**: 整合了 `check_spike_collision` 逻辑，确保陷阱伤害与移动逻辑紧密结合。

### ⚔️ 战斗与数值系统
**3. `combat_logic.pl` (战斗逻辑)**
- **功能**: 封装所有战斗判定与伤害结算逻辑，现已全面支持多参数敌人结构。
- **机制**:
  - **兼容性升级**: 适配了 `chaser` 的 6 参数结构（新增激活状态）和其他敌人的不同参数结构。
...
- **属性**: 初始 ATK 10，具有 **激活(Active)** 状态。
- **行为**: 
  - **待机模式**: 初始生成于 [10, 8]，处于休眠状态，不会移动。
  - **激活触发**: 当玩家进入特定区域 (X=16, Y=7~9) 时觉醒。
  - **追击模式**: 觉醒后使用 BFS 算法寻找最短路径追击玩家。若处于眩晕状态则跳过行动。
...
**9. `static_boss.pl` (原 hidden_bee.pl - 代号 'B')**
- **属性**: 初始 ATK 30，无法移动。
- **行为**: 
  - **周期攻击**: 每 5 回合向boss房中随机位置（同一横坐标）释放两列同步尖刺。
  - **尖刺机制**: 尖刺 (^) 每回合向右移动，触碰玩家（X坐标重合）造成 5 点伤害并消失。
